...
export class CssDocumentProvider extends VirtualDocumentProvider {
  public constructor(private readonly htmlLanguageService: LanguageService) {
    super();
  }

  public createVirtualDocument(context: TemplateContext): vscode.TextDocument {
    const regions = getDocumentRegions(
      this.htmlLanguageService,
      new VirtualDocumentProvider().createVirtualDocument(context)
    );
